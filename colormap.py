...
def sector_color_map(ALPHABET):
    ...
    for letter in ALPHABET:
        mapping[letter] = color.gray(init)
        init -= incr
    for number in NUMBERS:
        mapping[number] = color.gray(0.65)
    mapping[''] = color.rgb.red
    return mapping

def bezier_color_map(ALPHABET):
    incr = 1.0/(len(ALPHABET)*1.5)
    mapping = {}
    init = 1.0 - 0.5*(1.0-(len(ALPHABET)*incr) )
    for letter in ALPHABET:
        mapping[letter] = color.gray(init)
...
BEZIER_MAPPING = bezier_color_map(ALPHABET)
